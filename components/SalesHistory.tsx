...
import { Search, Calendar, Filter, Eye, X, Tag, FileText, CreditCard } from 'lucide-react';
...
export const SalesHistory: React.FC<SalesHistoryProps> = ({ sales }) => {
  ...
  const totalFiltered = filteredSales.reduce((acc, curr) => acc + curr.total, 0);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Relatório detalhado de transações.</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center">
          ...
          <div className="text-sm font-bold text-gray-700">
             Total no Período: <span className="text-green-600 ml-1 text-lg">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalFiltered)}</span>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-gray-800">
                    {sale.customerName}
                    <div className="text-xs text-gray-400">ID: {sale.id}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-block px-2 py-1 rounded-md text-xs font-bold border
                      ${sale.paymentMethod === 'MONEY' ? 'bg-green-50 text-green-700 border-green-200' : 
                        ...
                        'bg-orange-50 text-orange-700 border-orange-200' // Bemol
                      }
                    `}>
              ...
              {filteredSales.length === 0 && (
                <tr>
                   <td colSpan={5} className="text-center py-8 text-gray-400">Nenhuma venda encontrada no período.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="text-lg font-bold text-gray-800">Detalhes da Venda #{selectedSale.id}</h3>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="space-y-2 pt-2 border-t border-gray-100">
                 ...
                 <div className="flex justify-between text-lg font-bold text-purple-900 pt-2">
                 ...
                 {selectedSale.change && selectedSale.change > 0 && (
                    <div className="flex justify-between text-sm text-gray-500 pt-1">
                       ...
                       <span>{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedSale.change)}</span>
                    </div>
                 )}
              </div>
